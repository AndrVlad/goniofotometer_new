...
uint8_t rangle_angle_items[8] 	= 	{180,150,120,90,60,30,10,5};
...
void initDynamicMeasurement(meas_type_t meas_type, uint16_t start_angle_item, uint16_t end_angle_item, uint16_t step_item) {
	...
	start_angle = (rangle_angle_items[start_angle_item-1]);
	end_angle = 360 - start_ending_angle_items[end_angle_item-1];
	...
	if (full_rotation) {
		...
		if ((end_position_tmp - 720) < 0) {
			control_pos = ENCODER_RESOLUTION - abs(end_position_tmp - 720);
		} else {
			control_pos = end_position_tmp - 720;
		}

		measurement.end_pos = calculateEncPosition((((end_angle+4) * ENCODER_RESOLUTION) / 360));
	}

	setFSMGlobalState(EQUATORIAL_MEASUREMENT);
	setFSMActionState(ACCELERATION);

	setPlatformSpeed(measurement.meas_step, measurement.platform);
